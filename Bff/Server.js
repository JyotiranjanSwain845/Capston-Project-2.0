...
app.get("/api/incidents", async (req, res) => {
  ...
  try {
    const r = await axios.get(
      `${SN_INTANCE}/api/now/table/incident?sysparm_display_value=true&sysparm_fields=sys_id%2Cnumber%2Cstate%2Cpriority%2Cimpact%2Curgency%2Cshort_description`
,
      {
...
app.put("/api/incidents/:id", async (req, res) => {
  ...
  try {
    ...
    // Other errors
    if (e.response) {
      console.error("PUT error:", e.response.status, e.response.data);
      return res
        .status(e.response.status)
        .json({
